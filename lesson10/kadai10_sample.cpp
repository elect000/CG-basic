...
void updateCloth(void) {
	...
	for(int i = 0; i < cloth->springs.size(); i++) {// 全てのバネについて順番に処理する
		...
		float prelen = spring->restLength;
    float newlen = (spring->p0->p - spring->p1->p).length();
    float lendiff = (prelen - newlen);
		// (b) バネが質点に加える力の大きさを求める
		// （自然長 - 現在の長さ）にバネ定数 Ks を掛けた値が求める大きさ
    float powerval = lendiff * Ks;
